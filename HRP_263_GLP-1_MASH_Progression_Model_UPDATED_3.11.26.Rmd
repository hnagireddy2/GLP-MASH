...
# replaced by the best of the candidate parameter sets determined by the calibration below.
# Transitions to HCC, DCC, LT, death are fixed inputs and held constant throughout.
...
N_trial       <- 266
obs_regress   <- 59        # "true" observed regressions
obs_regress_c <- 50        # secondary: missing-biopsy-as-non-responder
weeks_trial   <- 72
...
run_single_start <- function(p_prog_local, start_state) {
  ...
  # Build full 4D array with no treatment effect (RR = 1 for all)
  aP <- build_a_P(
...
#   F2 starter (31.3%) regresses if ending in F0 or F1
#   F3 starter (68.7%) regresses if ending in F0, F1, or F2
# Single-start runs keep F2 and F3 trajectories separable
predict_regression <- function(p_prog_local) {
  ...
  expected_n  <- p_reg_total * N_trial

  ci_lo <- qbinom(0.025, N_trial, p_reg_total)
  ci_hi <- qbinom(0.975, N_trial, p_reg_total)
...
# ---- 7. Loop through all candidate sets ----
results <- data.frame(
  set        = character(),
  p_reg_F2   = numeric(),
  p_reg_F3   = numeric(),
...
for (nm in names(candidate_sets)) {
  ...
  results <- rbind(results, data.frame(
    ...
    ci_lo      = out$ci_lo,
    ci_hi      = out$ci_hi,
    hits_59    = (59 >= out$ci_lo) & (59 <= out$ci_hi),
    hits_50    = (50 >= out$ci_lo) & (50 <= out$ci_hi),
    diff_59    = round(out$expected_n - 59, 1)
  ))
}

cat("\n========== ESSENCE Calibration Results ==========\n")
cat("Observed regressions (true):         59 / 266\n")
cat("Observed regressions (missing = non-responder): 50 / 266\n\n")
print(results, row.names = FALSE)

# ---- 8. Pick the best-fitting parameter set ----
# Select the set whose predicted regression count most closely matches the missing-as-non-responder count of 50
results$diff_50 <- round(results$expected_n - 50, 1)
best <- results$set[which.min(abs(results$diff_50))]
cat("\nBest-fitting parameter set (closest to 50):", best, "\n")
...
ggplot(results, aes(x = set, y = expected_n)) +
  ...
  geom_hline(yintercept = 59, linetype = "dashed", linewidth = 0.8) +
  geom_hline(yintercept = 50, linetype = "dotted",  linewidth = 0.6) +
  annotate("text", x = 6.3, y = 59, label = "True = 59",
           hjust = 0, size = 3.5) +
  annotate("text", x = 6.3, y = 50, label = "missing as non-responders = 50",
           hjust = 0, size = 3.5) +
  labs(x     = "Transition Probabilities Source",
       y     = "Number of Individuals\nwith Fibrosis Regression",
       title = "ESSENCE Calibration: Candidate Parameter Sets vs Observed") +
...
target_hagstrom <- data.frame(
  ...
  time_low     = c(25, 20, 13, 7,  3),
  time_high    = c(35, 30, 22, 13, 7),
  source       = "Hagstrom et al. J Hepatol 2017"
)
...
time_to_severe <- function(start_state, threshold = 0.10) {
  ...
  for (t in 1:n_cycles) {
    m_tr[t + 1, ] <- m_tr[t, ] %*% aP_val[,,t,"LSM"]
  }
  
  # Cumulative proportion reaching "severe" (any liver-disease state)
  severe_states <- c("DCC", "HCC", "LT_Y1", "LT_Y1_P", "Post_LT")
  pct_severe    <- rowSums(m_tr[, severe_states])
  ...
  # Convert cycle to years
  years_at_threshold <- (cycle_at_threshold - 1) * cycle_length
  years_at_threshold
}
...
plot_severe_trajectory <- function() {
  ...
  ggplot(df_traj, aes(x = Year, y = PctSevere, color = Stage)) +
    ...
    scale_y_continuous("Cumulative proportion in severe disease",
...
summarize_outcomes <- function(trace, drug_cost_per_year,
  ...
  # Drug cost while on treatment
    # Conservative estimate: charge full annual cost for each calendar year the patient is treated (even if treatment ends partway through a year)
